...
import {
	...
	RequestConfig,
	type RequestConfig_NdlApiType,
	type RequestConfig_PickGroupType,
} from "@/api/v3/collo_pb";
import { createPromiseClient } from "@connectrpc/connect";
import { type NetworkHandle, type V3Response, transport } from "./connect";
...
export type NetworkState = Map<
	string,
	Pick<NetworkStreamResponse, "nodes" | "edges" | "meta">
>;

export const useNetworkState = () => {
	// networkデータ
	const [network, setNetwork] = useState<NetworkState>(new Map());
	const [requestParms, setRequestParams] = useState<NetworkStreamRequest>(
		new NetworkStreamRequest(),
	);
		...
		setNetwork((pns) => {
			...
			for (const [key, val] of pns.entries()) {
				...
				if (val.nodes.length > 0) {
					// uniq
					const reservedNodeID = [val.nodes[0].nodeId];
				...
				if (val.edges.length > 0) {
					// uniq
					const reservedEdgeID = [val.edges[0].edgeId];
