...
export type SubNetworkGraphProps = {
	...
	contentsProps: {
		metaProps: Array<
			NonNullablePick<React.ComponentProps<"a">, "href" | "children">
		>;
		loading: boolean;
		top3Button: NonNullablePick<
				...
				<div className="absolute top-1 z-10 w-full overflow-x-hidden max-w-lg mx-auto py-4 px-3 bg-white shadow-md rounded-md resize h-fit max-h-[580px]">
					...
					<div className="flex my-5">
						<div
							...
							className="mr-2 px-2 cursor-pointer rounded-sm hover:bg-red-100"
						>
							{"x"}
						</div>
					...
					{props.contentsProps.loading ? (
						...
						<StartButton
							type="button"
							value={"主要3単語のネットワークを取得する"}
							{...props.contentsProps.top3Button}
						/>
					)}
				</div>
			</div>
...
const GroupSelect = (props: GroupSelectProps) => {
	return (
		<select
			...
			className="w-full rounded border focus:outline-none focus:border-blue-500"
		>
...
type GroupOptionProps = NonNullablePick<
	React.ComponentProps<"option">,
	"key" | "value" | "children"
>;
const GroupOption = ({ children, ...props }: GroupOptionProps) => {
	return (
		<option {...props} className="px-2 py-1 w-full text-sm">
			{children}
		</option>
	);
};
