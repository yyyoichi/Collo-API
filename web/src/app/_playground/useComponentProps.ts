...
import { type RequestParamsFromUI, useNetworkState } from "./useNetworkState";
...
import type { NetworkHandle } from "./connect";
import {
...
export const useComponentProps = (): PlayGroundComponentProps => {
	...
	const { progress, loading, startLoading, stopLoading, ...stream } =
		useLoadingState(); // データ取得の進捗
	...
	const nwHandle: NetworkHandle = {
		...
		stream: (resp): void => {
			...
			if (resp instanceof NetworkStreamResponse) {
				...
				console.log(
					`Get ${resp.nodes.length}_Nodes, ${resp.edges.length}_Edges. At ${resp.meta?.groupId}`,
				);
			} else {
	...
	const getNetwrokLoaderProps: (
		id: string,
	) => PlayGroundComponentProps["networkProps"]["loaderProps"] = useCallback(
		(id: string) => {
			return {
				useLoadingGraphEffect: useLoadGraphEffect.bind(this, {
					...
					continueRequest: (forcusNodeID: number) => {
						return networkState.continueRequest(
							forcusNodeID,
							"total",
							nwHandle,
						);
	...
	const subNetworksProps: PlayGroundComponentProps["subNetworksProps"] =
		subnetworkState.groupIDs.map((id, at) => {
	...
	const selectedNodeProps: PlayGroundComponentProps["selectedNodeProps"] =
		useMemo(
			() =>
	...
	const updateNodeRankProps: PlayGroundComponentProps["updateNodeRankProps"] =
		useMemo(
			() => ({
				buttonProps: {
					...
					onClick: () => {
						...
						startLoading();
						rankGetterState
							.request(networkState.requestParms.config, nwHandle)
							.then((resp) => {
	...
	const props: PlayGroundComponentProps = {
		formProps: {
			onSubmit: (event) => {
				...
				const stopwords =
					form.get("stopwords")?.toString().trim().split(/\s+/) || [];
