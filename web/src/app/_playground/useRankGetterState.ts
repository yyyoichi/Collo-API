...
import { NetworkStreamRequest, Node, NodeRateStreamRequest, RequestConfig } from '@/api/v3/collo_pb';
import { MintGreenService } from '@/api/v3/collo_connect';
import { createPromiseClient } from '@connectrpc/connect';
import { NetworkHandle, transport } from './connect';
...
export const useRankGetterState = (params: NetworkStreamRequest) => {
  ...
  const request = async (config: RequestConfig | undefined, handle: NetworkHandle) => {
    handle.start();
    ...
    try {
      ...
      console.log(`Start request.. Offset:${req.offset},`, `Limit:${req.limit}`);
      let returnCount = 0;
