...
type DocMatrixInterface interface {
	...
	CoOccurrencetFrequency(coIndex [2]int) float64
	// 文書[dindex]における共起単語[coIndex]の共起頻度を取得する
	CoOccurrencetFrequencyAt(dindex int, coIndex [2]int) float64
}

// TF-IDFで重みを付けた出現単語行列
func NewTFIDFDocMatrix(docMatrix *CountDocMatrix) *DocMatrixBase[[]float64] {
	...
	m := newBaseDocMatrix(docMatrix.words, matrix)
	m.getValueAt = func(dindex, windex int) float64 {
...
type DocMatrixBase[T any] struct {
	...
	getValueAt func(dindex, windex int) float64
}
...
func (m *DocMatrixBase[T]) GetValueAt(dindex, windex int) float64 {
	return m.getValueAt(dindex, windex)
}
...
func (m *DocMatrixBase[T]) CoOccurrencetFrequency(coIndex [2]int) float64 {
	// 共起頻度
	frequency := 0.0
	// 共起関係が存在した文書の数
	count := 0
...
func (m *DocMatrixBase[T]) CoOccurrencetFrequencyAt(dindex int, coIndex [2]int) float64 {
	return m.getValueAt(dindex, coIndex[0]) * m.getValueAt(dindex, coIndex[1])
}
...
func (m *CountDocMatrix) createTFIDFDocMatric() DocMatrixInterface {
	matrix := make([][]float64, m.LenDocs())
	for dindex := range m.docs {
		matrix[dindex] = make([]float64, m.LenWords())
		for windex := 0; windex < m.LenWords(); windex++ {
			matrix[dindex][windex] = m.getTFIDFAt(dindex, windex)
		}
	}
	mm := newBaseDocMatrix(m.words, matrix)
	mm.getValueAt = func(dindex, windex int) float64 {
		return mm.docs[dindex][windex]
	}
	return mm
}
